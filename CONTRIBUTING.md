...
```bash
git clone git@github.com:sinemacula/http-primitives-php.git
cd http-primitives-php
composer install
```
...
```text
feat: add the Charset enum
fix: correct the reason phrase for 306 Unused
test: cover the HttpStatus range-classification boundaries
...
```bash
vendor/bin/phpunit tests/Unit/Enums/HttpStatusTest.php
vendor/bin/phpunit --filter testGetReasonPhrase tests/Unit/Enums/HttpStatusTest.php
```
